...
import { loadApplicationPrefill } from "@/server/loaders/application-prefill-loader.server";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const result = await loadApplicationPrefill(parsed.data);
    if (!result.ok) {
      return NextResponse.json(
    ...
    if (error instanceof PortalConfigError) {
      return NextResponse.json(
        { code: error.code, message: "Server configuration error." },
        { status: 500 },
      );
    }
